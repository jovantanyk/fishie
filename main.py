...
class Fish:
    ...
    def update_physics(self):
        ...
        # Update velocity based on acceleration and frame rate
        self.velocity += acceleration * (1/60)

    def modify_resistance(self, change):
        self.net_force += change  # Modify net force by resistance change

# Instantiate the fish centered horizontally and vertically
...
while running:
    ...
    if keys[pygame.K_UP]:
        fish.modify_resistance(-0.5)  # Increase resistance, decrease net force
    if keys[pygame.K_DOWN]:
        fish.modify_resistance(0.5)  # Decrease resistance, increase net force

    # Update fish physics
    fish.update_physics()

    # Scroll the background proportional to fish velocity
    background_offset -= fish.velocity * (1/60) * 10  # Adjusting factor for better visual effect
    background_offset %= background_width  # Wrap around using modulo to avoid large negative numbers
    ...
    for i in range(num_tiles):
        screen.blit(background_image, (i * background_width + background_offset - background_width, 0))
